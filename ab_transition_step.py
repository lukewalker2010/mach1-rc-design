...
def make_flange(od, id_, thick, pcd, bolt_dia, n_holes):
    flange = cq.Workplane("XY").circle(od / 2).circle(id_ / 2).extrude(thick)
    for i in range(n_holes):
        angle = i * 360.0 / n_holes
        x = pcd / 2 * math.cos(math.radians(angle))
        y = pcd / 2 * math.sin(math.radians(angle))
        flange = flange.faces("<Z").workhole(x, y, bolt_dia / 2).cutThruAll()
    return flange
...
def make_transition():
    ...
    body = body.circle((outlet_id + 2 * wall_t) / 2).workplane(offset=length).circle((inlet_id + 2 * wall_t) / 2).loft()
    inner = cq.Workplane("XY").transformed(offset=(0, 0, inlet_flg_t - 0.01))
    inner = inner.circle(outlet_id / 2).workplane(offset=length + 0.02).circle(inlet_id / 2).loft()
    body = body.cut(inner)
